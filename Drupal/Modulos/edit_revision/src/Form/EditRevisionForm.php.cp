...
class EditRevisionForm extends FormBase{
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $rev_id = NULL){
    ...
    if (!in_array(\Drupal::currentUser()->id(), $results) || !in_array('pentester', \Drupal::currentUser()->getRoles())){
    ...
    foreach ($results as $result) {
      ...
      $select->condition('id_revision',$result->id_rev_sitio);
      $datHall = $select->execute();
      foreach ($datHall as $hallazgo) {
        //Boton de editar
        $urlEditar = Url::fromRoute('asignar_hallazgos.content', array('rev_id' => $rev_id,'id_rev_sitio' => $result->id_rev_sitio));
      ...
      $url = Url::fromRoute('asignar_hallazgos.content', array('rev_id' => $rev_id,'id_rev_sitio' => $result->id_rev_sitio));
